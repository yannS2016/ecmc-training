...
# ecmc's own configure/RELEASE defines SUPPORT = \$(TOP)/.. -- a path relative
# to $ECMC_SRC that resolves to the same $EPICS_MODULES directory as ours, but
# as different literal text. EPICS's checkRelease compares SUPPORT text, not
# resolved paths, so the training IOC's build fails with "Definition of
# SUPPORT conflicts with ECMC support" unless ecmc's SUPPORT is overridden to
# match ours exactly. RELEASE.local always wins over RELEASE, so this is
# enough -- no edit to the checkout's own configure/RELEASE.
echo "==> writing $ECMC_SRC/configure/RELEASE.local"
cat > "$ECMC_SRC/configure/RELEASE.local" <<EOF
# GENERATED by 00-bootstrap/bootstrap.sh from site.conf -- do not edit.
# Re-run bootstrap.sh after changing site.conf.
#
# Overrides ecmc's own SUPPORT so it matches $app/configure/RELEASE.local
# literally, which is what EPICS's checkRelease requires.
SUPPORT  = $EPICS_MODULES
ETHERLAB = $ETHERLAB
RUCKIG   = $RUCKIG_DIR
EOF
