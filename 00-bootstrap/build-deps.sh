...
if [[ $LIST -eq 1 ]]; then
  ...
  for i in "${!names[@]}"; do
    d="$DEPS_DIR/${names[$i]}/$(dep_release "${names[$i]}")"
    if [[ -d "$d/.git" ]]; then
...
for i in "${!names[@]}"; do
  ...
  hdr "$name $version -> $(dep_release "$name") ($type)"
  release="$(dep_release "$name")"
  dir="$DEPS_DIR/$name/$release"
...
if [[ $DRY -eq 0 ]]; then
  lock="$here/deps.lock"
  {
    ...
    echo "# name        release       upstream      commit    path"
