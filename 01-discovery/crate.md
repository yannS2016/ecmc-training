...
| 2   | EL7062-0000 | 0x1b963052   | EL7062      | 03      | 2ch stepper 48V 3A; ch2 used, ch1 unused; CSV (see 03-motion-ioc/README.md s.3) |
...
- Axis 1 motor: bipolar stepper, wired **parallel**, on **EL7062 channel 2**
  (channel 1 unused). Datasheet ratings (parallel): 4.24 A/phase, 1.4 mH/phase
  (1400 uH), 0.4 Ohms/phase (400 mOhm), 1.8 deg/step. For safety, run well
  below the parallel current rating for now. All of the above --
  `CH_ID=2`/`CH_ID_OTHER=1`/`DRV_CH=02`, `I_MAX_MA=1000`, `I_STDBY_MA=100`,
  `U_NOM_MV=24000`, `L_COIL_UH=1400`, `R_COIL_MOHM=400` -- are baked into
  `st.cmd`'s own defaults, no override needed to run stage 1/2 as-is.
